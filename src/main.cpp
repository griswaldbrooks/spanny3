...
std::ostream& operator<<(std::ostream& os,
                         tl::expected<bin_occupancy, std::string> const& bin_maybe) {
  if (!bin_maybe.has_value()) {
    os << bin_maybe.error();
    return os;
  }
...
struct hardware : public hardware_interface {
  ...
  tl::expected<bin_occupancy, std::string> operator()(std::vector<joint_angles> const& path) const {
    ...
    std::ranges::for_each(path_degrees, [this](auto const& joint_angle) {
      ...
      auto set = serialize_newline(joint_angle);
      // std::cout << "serialized = " << set;
      serial_.write(set);
    });
    try {
...
struct arbiter_single {
  ...
  tl::expected<bin_occupancy, std::string> operator()(position_t const& goal) {
    ...
    auto const goto_home =
        inverse_kinematics(plan_cartesian({goal, near_goal, home_}, 0.05), model_);
    // std::cout << "goto home\n";
...
int main() {
  std::ifstream bin_file{"src/spanny3/config/bin_config.json"};
  ...
  //           continue;
  //       }
  //       std::cout << "T = {" << right_arm.origin.x << ", " << right_arm.origin.y << ", " <<
  //       right_arm.orientation << "}" << std::endl;
  //       auto arbiter = arbiter_single{right_arm, std::make_unique<hardware>("/dev/ttyACM0",
  //       9600)};
  //       arbiter({0.210, 0.068});
  //       arbiter({0.210, 0.194});
  //       arbiter({0.273, 0.194});
  ...
  using hilbert_view_t = std::mdspan<double, std::extents<std::size_t, 3, 3>, std::layout_right,
